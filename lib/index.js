...
function classifyFailureText(text) {
  if (!text || !FAIL_MARKERS.some((m) => text.includes(m))) return null;
  if (text.includes("Model/connection initialization did not complete") || text.includes("huggingface.co")) {
    return "模型加载失败：无法访问 HuggingFace（离线模式未生效或网络不可达），模型初始化超时";
  }
  if (text.includes("WinError 10061")) return "端口 9077 被占用或服务未就绪";
  if (text.includes("WinError 10060")) return "网络连接超时（无法访问 HuggingFace）";
